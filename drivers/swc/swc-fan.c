...
static int swc_fan_probe(struct platform_device *pdev)
{
	...
	if (PTR_ERR(data->pwm) == -ENODEV) {
		...
		err = acpi_node_get_property_reference(pdev->dev.fwnode,
						      "pwm", 0, &ref);
		if (err) {
	...
	data->alarm = devm_gpiod_get(&pdev->dev, "alarm", GPIOD_ASIS);
	...
	data->present = devm_gpiod_get(&pdev->dev, "present", GPIOD_ASIS);
